...
- A toggle for the camera tilt that vanilla applies while you are taking damage.
- Toggles for the boss health bar, the scoreboard sidebar, and potion effect icons. These go through Forge's own HUD events and flags, so they behave the same on Celeritas, on Actinium, and without either.
- FPS, coordinate, and memory overlays with configurable position and text contrast. Extended FPS statistics include average FPS, 1% low, and 0.1% low.
...
- Code, tests and documentation in this fork — the dual-renderer support, the option-page
  specification the renderers are adapted to, the rename that came with it, and the newer option
  pages and HUD toggles — were generated by an AI agent working to the maintainer's instructions.
