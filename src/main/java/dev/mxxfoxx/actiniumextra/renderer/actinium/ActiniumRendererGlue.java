...
import dev.mxxfoxx.actiniumextra.client.gui.spec.ActiniumExtraOptionSpecs;
import dev.mxxfoxx.actiniumextra.client.gui.spec.FormatterSpec;
import dev.mxxfoxx.actiniumextra.client.gui.spec.GateSpec;
import dev.mxxfoxx.actiniumextra.client.gui.spec.GroupSpec;
import dev.mxxfoxx.actiniumextra.client.gui.spec.OptionSpec;
import dev.mxxfoxx.actiniumextra.client.gui.spec.PageSpec;
import dev.mxxfoxx.actiniumextra.client.gui.spec.StandardRewriteSpec;
import dev.mxxfoxx.actiniumextra.client.gui.spec.TextSpec;
import dev.mxxfoxx.actiniumextra.renderer.RendererGlue;
...
/**
 * Binds Actinium Extra to Actinium's option model ({@code dhj.embeddedt.embeddium.api}).
 * <p>
 ...
 * every row Actinium Extra contributes is converted from the shared specification here. This class
 ...
 * {@link dev.mxxfoxx.actiniumextra.renderer.RendererBackend} once Actinium has been detected, so a
 ...
 * and the shared policy stops Actinium Extra from contributing a second screen-mode control.
 */
public final class ActiniumRendererGlue implements RendererGlue {
    ...
    private void onGuiConstruct(OptionGUIConstructionEvent event) {
        ...
        ActiniumExtraMod.LOGGER.info("Registered {} Actinium Extra option pages", pages.size());
    }
    ...
    private void onGroupConstruct(OptionGroupConstructionEvent event) {
        ...
        List<StandardRewriteSpec> rewrites =
                ActiniumExtraOptionSpecs.windowRewrites(shouldProvideScreenMode());
        ...
        for (int i = 0; i < options.size(); i++) {
            ...
            for (StandardRewriteSpec rewrite : rewrites) {
                ...
                ActiniumExtraMod.LOGGER.debug("Replaced the Actinium {} option with the Actinium Extra control",
                        target);
            }
        }
    }
    ...
    /**
     * Maps an Actinium option identifier onto the row Actinium Extra knows how to replace.
     *
     * @param id the identifier of a row in the WINDOW group, possibly {@code null}
     * @return the targeted row, or {@code null} when Actinium Extra has no replacement for it
     */
    ...
    private OptionPage toPage(PageSpec page) {
        ...
        for (GroupSpec group : page.groups()) {
            ...
            group.titleText().ifPresent(title -> builder.setName(text(title)));
    ...
    private Option<?> toBool(OptionSpec.Bool spec, Map<String, Option<?>> built,
                             StandardRewriteSpec.StandardId standard) {
        OptionImpl.Builder<ActiniumExtraGameOptions, Boolean> builder = OptionImpl
     ...
     */
    private <T> void applyShared(OptionImpl.Builder<ActiniumExtraGameOptions, T> builder,
