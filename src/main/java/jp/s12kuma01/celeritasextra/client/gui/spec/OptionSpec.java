 ...
 * names, and each renderer only accepts its own classes in its GUI, so the pages Celeritas Extra
 * contributes are described here first and turned into renderer objects by the matching adapter in
 * {@code jp.s12kuma01.celeritasextra.renderer}. Every record binds straight to fields of
 * {@link CeleritasExtraGameOptions} through the same getter/setter pairs the pre-adapter code used.
 * <p>
...
public sealed interface OptionSpec {
    ...
    static <T extends Enum<T>> Cycling<T> cycling(String key,
                                                 ...
                                                 BiConsumer<CeleritasExtraGameOptions, T> setter,
                                                 Function<CeleritasExtraGameOptions, T> getter) {
